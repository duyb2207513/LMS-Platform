...
<template>
  <a
    :href="githubLoginUrl"
    class="group relative flex min-h-12 w-full items-center justify-center gap-3 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-800 hover:shadow-md active:translate-y-0 focus:outline-none focus:ring-4 focus:ring-slate-300 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 dark:focus:ring-slate-700"
    aria-label="Tiếp tục với GitHub"
  >
    <svg viewBox="0 0 24 24" class="h-5 w-5 transition-transform duration-200 group-hover:scale-110" aria-hidden="true" fill="currentColor">
    ...
    <span class="tracking-wide">Tiếp tục với GitHub</span>
  </a>
</template>
