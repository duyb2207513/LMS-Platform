...
const mainLinkClass = (path: string) => {
  ...
  return [
    ...
    active
      ? "bg-white text-violet-700 shadow-md shadow-black/10"
...
<template>
  <header
    class="sticky top-0 z-40 border-b border-violet-800/80 bg-gradient-to-r from-indigo-800 via-violet-700 to-purple-700 text-white shadow-md"
  >
    <div
    ...
    >
      ...
      <div class="flex shrink-0 items-center gap-1.5 sm:gap-2">
        <RouterLink
          ...
          :to="dashboardPath()"
          class="hidden h-10 items-center gap-1.5 rounded-xl border border-white/20 bg-white/10 px-3 text-xs font-bold text-white shadow-sm backdrop-blur-md transition hover:bg-white/20 md:inline-flex"
        >
        ...
        <div v-else-if="!auth.isStudent" class="relative">
          <button
            type="button"
            class="flex min-h-10 items-center gap-2 rounded-xl border border-white/20 bg-white/10 p-1.5 pr-1.5 text-left text-white shadow-sm backdrop-blur-md transition hover:bg-white/20 sm:pr-3"
          ...
          >
            ...
            <span
              v-else
              class="grid h-8 w-8 place-items-center rounded-full bg-white text-xs font-black text-violet-700 shadow-sm"
              >{{ auth.userInitials }}</span
            >
            <span
              class="hidden text-sm font-bold text-white sm:block whitespace-nowrap"
              >{{ auth.user?.fullName }}</span
            >
            <svg
              class="hidden h-4 w-4 text-violet-200 sm:block"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
          ...
          <Transition name="dropdown">
            <div
              v-if="profileOpen"
              class="absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-900/15 dark:border-slate-700 dark:bg-slate-900"
              role="menu"
            >
              <div
        ...
        <button
          v-else
          type="button"
          class="inline-flex min-h-10 items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-3 text-sm font-bold text-white backdrop-blur-md transition hover:bg-red-600 hover:border-red-600 sm:px-4"
          @click="logout"
        >
        ...
        <button
          type="button"
          :class="[
            'grid h-10 w-10 place-items-center rounded-xl border border-white/20 bg-white/10 text-white shadow-sm backdrop-blur-md transition hover:bg-white/20',
    ...
    <Transition name="mobile-nav">
      <nav
        ...
        class="border-t border-violet-800 bg-slate-900 px-3 py-3 shadow-2xl md:hidden"
      ...
      >
        <div class="mx-auto grid max-w-7xl gap-1">
          ...
          <RouterLink
            v-if="auth.isAdmin || auth.isInstructor"
            :to="dashboardPath()"
            class="rounded-xl bg-white/10 px-4 py-2 text-sm font-bold text-purple-200"
            >⚙️ {{ auth.isAdmin ? "Vào trang Quản trị" : "Vào trang Giảng viên" }}</RouterLink
          >
          <RouterLink
            v-if="!auth.isLoggedIn"
            to="/login"
            class="rounded-xl px-4 py-2 text-sm font-semibold text-white hover:bg-white/10"
