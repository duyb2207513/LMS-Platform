...
export interface SidebarNavItem { label: string; to: string; icon: string }
const props = withDefaults(defineProps<{ items: SidebarNavItem[]; open?: boolean }>(), { open: false })
const emit = defineEmits<{ close: [] }>()
const route = useRoute(), auth = useAuthStore()
const activePath = computed(() => props.items.filter((item) => route.path === item.to || route.path.startsWith(`${item.to}/`)).sort((left, right) => right.to.length - left.to.length)[0]?.to)
const roleLabel = computed(() => auth.isAdmin ? 'Quản trị viên' : auth.isInstructor ? 'Giảng viên' : 'Học viên')
function linkClasses(path: string) { return ['group flex min-h-11 items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition-[background-color,color,box-shadow,transform] duration-200', activePath.value === path ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-md shadow-purple-500/20' : 'text-slate-600 hover:translate-x-0.5 hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white'] }
</script>
...
<template>
  ...
  <aside :class="['fixed inset-y-0 left-0 z-50 flex w-[17rem] flex-col border-r border-slate-200 bg-white shadow-2xl shadow-slate-950/15 transition-transform duration-300 dark:border-slate-800 dark:bg-slate-900 lg:sticky lg:top-16 lg:z-20 lg:h-[calc(100vh-4rem)] lg:w-[17rem] lg:translate-x-0 lg:shadow-none', open ? 'translate-x-0' : '-translate-x-full']" aria-label="Điều hướng quản lý">
    <div class="flex h-16 items-center justify-between border-b border-slate-100 px-4 dark:border-slate-800 lg:hidden">
...
<style scoped>.fade-enter-active,.fade-leave-active{transition:opacity 200ms ease}.fade-enter-from,.fade-leave-to{opacity:0}</style>
