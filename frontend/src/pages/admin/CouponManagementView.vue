...
<template>
  <AdminLayout>
    <div class="space-y-6">
      <header class="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-slate-500">
            Tạo, chỉnh sửa và quản lý coupon ưu đãi cho khóa học
          </p>
        </div>
      ...
      >
        <div class="flex flex-1 items-center gap-3 max-w-md">
          ...
          />
          <BaseButton variant="secondary" size="sm" @click="loadCoupons"
            >Tìm</BaseButton
          >
        </div>
      ...
      <div class="surface-card overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            <thead
              class="bg-slate-50 text-xs uppercase font-bold tracking-wider text-slate-500 dark:bg-slate-900/60 dark:text-slate-400"
            >
              <tr>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              >
                <td class="px-5 py-4">
                  <div
                    class="font-mono font-extrabold text-purple-700 dark:text-purple-300"
                  >
                    {{ c.code }}
                  </div>
                ...
                <td class="px-5 py-4 font-semibold">
                  <span
                    v-if="c.discountType === DiscountType.PERCENTAGE"
                    class="text-purple-600 dark:text-purple-400"
                  >
                    Giảm {{ c.discountValue }}%
                    <span
                      v-if="c.maxDiscountAmount"
                ...
                <td class="px-5 py-4 text-xs">
                  <span
                    v-if="c.appliesToAllCourses"
                    class="rounded bg-slate-100 px-2 py-0.5 font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300"
                    >Tất cả khóa học</span
                  >
                  <span
                    v-else
                    class="rounded bg-purple-50 px-2 py-0.5 font-bold text-purple-700 dark:bg-purple-950/60 dark:text-purple-300"
                    >{{ c.courses?.length || c.courseIds?.length || 0 }} khóa
                    học</span
                  >
                </td>
    ...
    >
      <form class="space-y-4" @submit.prevent="handleSaveCoupon">
        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            <label
              class="mb-1 block text-xs font-bold text-slate-600 dark:text-slate-300"
              >Mã Coupon *</label
            >
            <input
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label
              class="mb-1 block text-xs font-bold text-slate-600 dark:text-slate-300"
              >Tổng số lượt sử dụng tối đa</label
            >
            <input
        ...
        <div>
          ...
          >
            <input
              v-model="formData.appliesToAllCourses"
              type="checkbox"
              class="h-4 w-4 rounded text-purple-600"
            />
        ...
        <div v-if="!formData.appliesToAllCourses">
          ...
          >
            <option
              v-for="crs in courseStore.courses"
              :key="crs.id"
              :value="crs.id"
            >
              {{ crs.title }}
        ...
        <p v-if="formError" class="text-xs font-bold text-red-600">
          {{ formError }}
        </p>
        ...
        <div class="flex justify-end gap-3 pt-3">
          <BaseButton
            ...
            >Hủy</BaseButton
          >
          <BaseButton type="submit" :loading="couponApi.loading.value"
            >Lưu Coupon</BaseButton
          >
        </div>
      </form>
    </BaseModal>
    ...
    >
      ...
      <div
        v-else-if="!selectedCouponUsages.length"
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <tr v-for="u in selectedCouponUsages" :key="u.id">
              ...
              <td class="p-3 font-bold text-emerald-600">
                {{ formatMoney(u.discountAmount) }}
              </td>
              <td class="p-3 text-slate-400">
                {{ formatDate(u.usedAt || u.createdAt) }}
              </td>
            </tr>
          </tbody>
        </table>
