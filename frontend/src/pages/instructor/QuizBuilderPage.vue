...
const api = useApi()
const lessonId = route.params.lessonId ? String(route.params.lessonId) : ''
const sectionId = route.params.sectionId ? String(route.params.sectionId) : ''
...
const questionImageKey = ref(0)
const questionImagePreview = computed(() => questionImage.value ? URL.createObjectURL(questionImage.value) : (!removeExistingImage.value ? editingQuestion.value?.imageUrl || '' : ''))
...
async function saveQuiz() {
  ...
  try {
    ...
    const createPath = sectionId ? `/sections/${sectionId}/quizzes` : `/lessons/${lessonId}/quizzes`
    const response = quiz.value ? await api.patch<ApiResponse<Quiz>>(`/quizzes/${quiz.value.id}`, payload) : await api.post<ApiResponse<Quiz>>(createPath, payload)
...
function resetQuestionForm() { questionForm.text = ''; questionForm.explanation = ''; questionForm.points = 1; questionForm.options = [{ text: '', isCorrect: true }, { text: '', isCorrect: false }]; removedOptionIds.value = []; questionImage.value = null; removeExistingImage.value = false; questionImageKey.value += 1 }
...
function openEditQuestion(question: QuizQuestion) {
  ...
  removedOptionIds.value = []; questionImage.value = null; removeExistingImage.value = false; questionImageKey.value += 1; questionModalOpen.value = true; error.value = ''
}
function selectQuestionImage(event: Event) { const file = (event.target as HTMLInputElement).files?.[0] || null; if (file && file.size > 5 * 1024 * 1024) { error.value = 'Ảnh câu hỏi không được vượt quá 5 MB'; return } questionImage.value = file; removeExistingImage.value = false }
function clearQuestionImage() { questionImage.value = null; removeExistingImage.value = Boolean(editingQuestion.value?.imageUrl); questionImageKey.value += 1 }
...
async function saveQuestion() {
  ...
  try {
    ...
    if (removeExistingImage.value) await api.del(`/questions/${questionId}/image`)
    if (questionImage.value) { const body = new FormData(); body.append('image', questionImage.value); await api.post(`/questions/${questionId}/image`, body) }
...
<template>
  <InstructorLayout>
    ...
    <BaseModal :show="questionModalOpen" :title="editingQuestion ? 'Chỉnh sửa câu hỏi' : 'Thêm câu hỏi'" description="Có thể đính kèm một hình minh họa JPG, PNG hoặc WebP." size="lg" @close="closeQuestionModal"><form class="space-y-5" @submit.prevent="saveQuestion">
      <div class="space-y-2"><label for="question-text" class="text-sm font-semibold">Nội dung câu hỏi</label><textarea id="question-text" v-model="questionForm.text" rows="3" class="quiz-textarea" placeholder="Nhập nội dung câu hỏi..." /></div>
      <div class="border border-dashed border-slate-300 p-4"><div class="flex items-center justify-between gap-3"><div><b class="text-sm">Hình minh họa</b><p class="text-xs text-slate-500">JPG, PNG, WebP · tối đa 5 MB</p></div><label class="cursor-pointer border border-slate-300 px-3 py-2 text-sm font-bold">Chọn ảnh<input :key="questionImageKey" type="file" accept="image/jpeg,image/png,image/webp" class="sr-only" @change="selectQuestionImage"></label></div><div v-if="questionImagePreview" class="mt-3 flex items-start gap-3"><img :src="questionImagePreview" alt="Xem trước câu hỏi" class="h-32 max-w-xs border object-contain"><button type="button" class="text-sm font-bold text-red-600" @click="clearQuestionImage">Gỡ ảnh</button></div></div>
      <div class="grid gap-4 sm:grid-cols-[1fr_140px]"><BaseInput id="question-explanation" v-model="questionForm.explanation" label="Giải thích sau khi chấm" placeholder="Không bắt buộc" /><BaseInput id="question-points" v-model="questionForm.points" type="number" label="Số điểm" required /></div>
      <div><div class="mb-3 flex items-center justify-between"><label class="text-sm font-semibold">Các đáp án</label><button type="button" class="text-sm font-bold text-purple-600" @click="addDraftOption">+ Thêm đáp án</button></div><div class="space-y-2"><div v-for="(option, index) in questionForm.options" :key="option.id || index" class="draft-option"><button type="button" :class="['correct-radio', option.isCorrect ? 'correct-radio--active' : '']" @click="makeCorrect(index)">{{ option.isCorrect ? '✓' : String.fromCharCode(65 + index) }}</button><input v-model="option.text" class="min-w-0 flex-1 bg-transparent text-sm outline-none" :placeholder="`Đáp án ${String.fromCharCode(65 + index)}`" /><button v-if="questionForm.options.length > 2" type="button" class="text-sm font-bold text-red-500" @click="removeDraftOption(index)">Xóa</button></div></div></div>
      <p v-if="error" class="bg-red-50 p-3 text-sm text-red-700">{{ error }}</p><div class="flex justify-end gap-3"><BaseButton type="button" variant="secondary" @click="closeQuestionModal">Hủy</BaseButton><BaseButton type="submit" :loading="questionSaving">{{ editingQuestion ? 'Lưu câu hỏi' : 'Thêm câu hỏi' }}</BaseButton></div>
    </form></BaseModal>
