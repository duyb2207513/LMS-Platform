...
import { ArrowLeft, ArrowRight, Megaphone } from '@lucide/vue'
  ...
  <DefaultLayout>
    ...
    <main v-else-if="course">
      <section class="course-hero text-white">
        <div class="course-detail-width course-hero-layout grid gap-10 lg:grid-cols-[minmax(0,1fr)_480px]">
          <div class="course-hero-copy">
            <RouterLink to="/courses" class="grid h-9 w-9 place-items-center border border-purple-300 text-purple-100 hover:bg-purple-700 hover:text-white" aria-label="Quay lại danh sách khóa học" title="Quay lại danh sách khóa học"><ArrowLeft :size="18" /></RouterLink>
            <div class="course-hero-primary">
              <div class="flex flex-wrap gap-2"><span class="hero-chip">{{ course.category?.name || 'Khóa học' }}</span><span class="hero-chip">{{ levelLabels[course.level] }}</span><span class="hero-chip">{{ course.language }}</span></div>
              <h1 class="mt-5 max-w-4xl text-4xl font-black leading-tight tracking-[-0.04em] sm:text-5xl lg:text-6xl">{{ course.title }}</h1>
              <p class="mt-5 max-w-4xl text-base leading-7 text-purple-100 sm:text-lg">{{ course.description }}</p>
            </div>
            <div class="course-hero-meta flex flex-wrap items-center gap-4 text-sm">
              <div class="flex items-center gap-2"><span class="grid h-9 w-9 place-items-center bg-violet-500 font-bold">{{ course.instructor?.fullName?.charAt(0) || 'L' }}</span><span>Giảng viên <b>{{ course.instructor?.fullName }}</b></span></div>
              <span class="hidden h-5 w-px bg-purple-300 sm:block" /><div class="flex items-center gap-2"><StarRating :model-value="Math.round(summary.averageRating)" readonly size="sm" /><b>{{ summary.averageRating.toFixed(1) }}</b><span class="text-purple-200">({{ summary.totalReviews }} đánh giá)</span></div>
            </div>
          </div>

          <aside class="course-purchase-card self-center overflow-hidden border border-white bg-white p-3 text-slate-950 shadow-xl dark:border-slate-700 dark:bg-slate-900 dark:text-white">
            <CourseThumbnail :src="course.thumbnailUrl" :alt="course.title" />
            <div class="p-3 sm:p-4">
              <div class="flex items-center justify-between gap-3"><p class="text-3xl font-black text-purple-700 dark:text-purple-300">{{ course.isFree ? 'Miễn phí' : money(course.price) }}</p><span v-if="enrollment" class="border border-emerald-300 bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">Đã đăng ký</span></div>
              <BaseButton class="mt-5" :full-width="true" size="lg" :loading="working" @click="primaryAction">{{ actionLabel }} <ArrowRight :size="18" /></BaseButton>
              <div v-if="enrollment" class="mt-4 flex flex-col gap-2">
                ...
                <RouterLink :to="`/courses/${course.id}/announcements`" class="flex items-center justify-center gap-1.5 text-center text-sm font-semibold text-slate-600 hover:text-purple-600 dark:text-slate-400"><Megaphone :size="16" /> Xem thông báo từ giảng viên</RouterLink>
              </div>
              <p v-if="actionError" class="mt-4 border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-900 dark:bg-red-950 dark:text-red-300">{{ actionError }}</p>
      ...
      <section class="course-detail-width grid gap-8 py-12 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div class="space-y-6">
      ...
      <section class="course-detail-width pb-16">
        ...
        <p v-else-if="auth.isStudent" class="mb-6 border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-300">Bạn cần đăng ký khóa học trước khi gửi đánh giá.</p>
        <div v-if="reviews.length" class="grid gap-4 md:grid-cols-2"><article v-for="review in reviews" :key="review.id" class="surface-card p-5"><div class="flex items-center justify-between gap-3"><div class="flex items-center gap-3"><span class="grid h-10 w-10 place-items-center bg-purple-100 font-bold text-purple-700 dark:bg-purple-950 dark:text-purple-300">{{ review.user.fullName.charAt(0) }}</span><div><b>{{ review.user.fullName }}</b><p class="text-xs text-slate-500">{{ new Date(review.createdAt).toLocaleDateString('vi-VN') }}</p></div></div><StarRating :model-value="review.rating" readonly size="sm" /></div><p class="mt-4 leading-7 text-slate-600 dark:text-slate-300">{{ review.content || 'Học viên chưa để lại nhận xét.' }}</p></article></div>
...
<style scoped>
.course-hero { background: linear-gradient(125deg, #312e81 0%, #6d28d9 52%, #9333ea 100%); }
.course-detail-width{width:100%;max-width:none;margin:0;padding-inline:.5rem}
.course-hero-layout{min-height:calc(100vh - 4.5rem);padding-top:5px;padding-bottom:1rem}
.course-hero-copy{display:grid;min-height:100%;grid-template-rows:auto 1fr auto}
.course-hero-primary{align-self:center;padding-block:2rem}
.course-hero-meta{align-self:end;padding-bottom:.25rem}
.hero-chip { border: 1px solid #c4b5fd; background: #6d28d9; padding: .4rem .8rem; font-size: .75rem; font-weight: 700; }
.course-purchase-card :deep(button),.course-purchase-card :deep(.course-thumbnail){border-radius:0!important}
...
.review-textarea { margin-top: 1.25rem; width: 100%; resize: vertical; border: 1px solid var(--border); border-radius: 1rem; background: var(--surface-muted); padding: 1rem; color: var(--text); outline: none; }.review-textarea:focus{border-color:#a855f7;box-shadow:0 0 0 4px #e9d5ff}
@media(max-width:1023px){.course-hero-layout{min-height:auto;padding-bottom:1.5rem}.course-hero-copy{min-height:34rem}.course-purchase-card{width:100%;max-width:32rem;justify-self:center}}
</style>
