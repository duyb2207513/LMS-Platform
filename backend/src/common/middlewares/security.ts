...
export const apiRateLimiter: RequestHandler = rateLimit({
  ...
  handler: (_request, res) => { res.status(429).json(response("Too many requests, please try again later")); }
});
export const authRateLimiter: RequestHandler = rateLimit({
  ...
  standardHeaders: "draft-7",
  legacyHeaders: false,
  handler: (_request, res) => { res.status(429).json(response("Too many authentication attempts, please try again later")); }
});
