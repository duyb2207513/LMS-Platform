...
const remove = (url: string) => unlink(path.join(LESSON_FILE_DIRECTORY, path.basename(url))).catch(() => undefined);
export async function createLessonController(request: Request, response: Response) { sendSuccess(response, 201, "Lesson created successfully", await createLesson(param(request, "sectionId"), request.auth, request.body as CreateLessonInput)); }
...
export async function deleteLessonController(request: Request, response: Response) { const urls = await deleteLesson(param(request, "lessonId"), request.auth); await Promise.all(urls.map(remove)); response.status(204).send(); }
export async function uploadLessonFileController(request: Request, response: Response) {
  ...
  const fileUrl = `${request.protocol}://${request.get("host")}/uploads/lesson-files/${request.file.filename}`;
  try {
