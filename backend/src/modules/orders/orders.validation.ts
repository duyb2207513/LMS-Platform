...
export function validateCreateOrderInput(body: unknown): RequestValidationResult<CreateOrderInput> {
  ...
  const input = body as Record<string, unknown>, errors: Record<string, string> = {}; for (const key of Object.keys(input)) if (key !== "courseIds" && key !== "couponCode") errors[key] = `${key} cannot be set through this endpoint`;
  ...
  if (input.couponCode !== undefined && (typeof input.couponCode !== "string" || input.couponCode.trim().length < 2 || input.couponCode.trim().length > 50)) errors.couponCode = "couponCode must contain 2 to 50 characters";
  return Object.keys(errors).length ? { errors } : { data: { courseIds: input.courseIds as string[], ...(typeof input.couponCode === "string" ? { couponCode: input.couponCode.trim().toUpperCase() } : {}) } };
}
