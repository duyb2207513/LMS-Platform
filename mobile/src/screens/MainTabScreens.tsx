...
import { Alert, Animated, PanResponder, Pressable, ScrollView, StyleSheet, Switch, Text, View } from 'react-native';
...
export function SearchScreen({ navigation }: { navigation: any }) {
  ...
  return <View style={styles.page}><Screen topInset scroll={false} quickScroll={false}>
    ...
    <View style={styles.searchBody}><Field label="Từ khóa" value={query} onChangeText={setQuery} placeholder="Tên khóa học, chủ đề..." returnKeyType="search" onSubmitEditing={search} />
      {loading || error || !items.length ? <StateView loading={loading} error={error} empty="Không tìm thấy khóa học phù hợp" onRetry={search} /> : <ScrollView showsVerticalScrollIndicator={false}>{items.map(item => <CourseCard key={item.id} course={item} onPress={() => navigation.navigate('CourseDetail', { slug: item.slug })} />)}</ScrollView>}
    </View>
...
export function NotificationsScreen({ navigation }: { navigation: any }) {
  ...
  return <View style={styles.page}><Screen topInset refreshing={loading} onRefresh={load}>
    <AppBar title="Thông báo" subtitle={`${items.filter(item => !item.isRead).length} chưa đọc`} onMore={openPreferences} />
    ...
    <View style={styles.markRow}><Text style={styles.swipeHint}>Vuốt trái để xóa · chạm để đánh dấu đã đọc</Text><Text style={styles.markAll} onPress={async () => { await notificationsApi.markAllRead(); await load(); }}>Đọc tất cả</Text></View>
    {loading || error || !items.length ? <StateView loading={loading} error={error} empty="Bạn chưa có thông báo nào" onRetry={load} /> : items.map(item => <SwipeNotification key={item.id} item={item} onPress={() => void read(item)} onDelete={() => void remove(item)} />)}
