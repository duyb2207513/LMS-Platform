...
export function CourseDetailScreen({ route, navigation }: NativeStackScreenProps<RootStackParamList, 'CourseDetail'>) {
  ...
  return <View style={styles.detailPage}><Screen>
    {course.thumbnailUrl ? <Image source={{ uri: course.thumbnailUrl }} style={styles.heroImage} /> : <View style={[styles.heroImage, styles.placeholder]}><Ionicons name="book-outline" size={58} color={colors.primary} /></View>}
    ...
    <View style={styles.instructorRow}><View style={styles.instructorAvatar}>{course.instructor?.avatarUrl ? <Image source={{ uri: course.instructor.avatarUrl }} style={styles.instructorAvatar} /> : <Text style={styles.instructorLetter}>{course.instructor?.fullName?.charAt(0) || 'L'}</Text>}</View><View style={{ flex: 1 }}><Text style={styles.byLabel}>Giảng viên</Text><Text style={styles.byName}>{course.instructor?.fullName || 'LMS Platform'}</Text></View><View style={styles.ratingPill}><Ionicons name="star" size={15} color={colors.warning} /><Text style={styles.ratingPillText}>{reviewSummary.averageRating.toFixed(1)} ({reviewSummary.totalReviews})</Text></View></View>
    ...
    <View style={styles.tabs}><Pressable onPress={() => setTab('overview')} style={[styles.tab, tab === 'overview' && styles.tabActive]}><Text style={[styles.tabText, tab === 'overview' && styles.tabTextActive]}>Tổng quan</Text></Pressable><Pressable onPress={() => setTab('reviews')} style={[styles.tab, tab === 'reviews' && styles.tabActive]}><Text style={[styles.tabText, tab === 'reviews' && styles.tabTextActive]}>Đánh giá</Text></Pressable></View>
    {tab === 'overview' ? <>
      ...
      <View style={styles.reviewSummaryBox}><Text style={styles.ratingNumber}>{reviewSummary.averageRating.toFixed(1)}</Text><View><Text style={styles.starsSmall}>{'★'.repeat(Math.round(reviewSummary.averageRating))}{'☆'.repeat(5 - Math.round(reviewSummary.averageRating))}</Text><Text style={styles.byLabel}>{reviewSummary.totalReviews} đánh giá từ học viên</Text></View></View>
      {user?.role === 'STUDENT' && enrollment && <View style={styles.reviewForm}><Text style={styles.reviewLabel}>Trải nghiệm của bạn</Text><Stars value={rating} onChange={setRating} /><Field label="Nhận xét" value={reviewText} onChangeText={setReviewText} multiline placeholder="Chia sẻ trải nghiệm học tập..." /><Button title={reviews.some(item => item.user.id === user.id) ? 'Cập nhật đánh giá' : 'Gửi đánh giá'} onPress={saveReview} loading={savingReview} disabled={!rating} /></View>}
      {reviews.length ? reviews.map(review => <View key={review.id} style={styles.review}><View style={styles.reviewTop}><Text style={styles.reviewer}>{review.user.fullName}</Text><Text style={styles.starsSmall}>{'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}</Text></View>{!!review.content && <Text style={styles.body}>{review.content}</Text>}</View>) : <StateView empty="Khóa học chưa có đánh giá" />}
    </>}
    <View style={{ height: 92 }} />
  </Screen>
  <View style={styles.purchaseDock}><View style={{ flex: 1 }}><Text style={styles.dockLabel}>{enrollment ? 'Tiến độ của bạn' : course.isFree ? 'Học phí' : 'Giá khóa học'}</Text><Text style={styles.dockPrice}>{enrollment ? `${Math.round(enrollment.progressPercent)}% hoàn thành` : course.isFree ? 'Miễn phí' : money(course.price)}</Text></View><View style={styles.dockAction}>
...
function Badge({ icon, label }: { icon: keyof typeof Ionicons.glyphMap; label: string }) { return <View style={styles.badge}><Ionicons name={icon} size={14} color={colors.primary} /><Text style={styles.badgeText}>{label}</Text></View>; }
