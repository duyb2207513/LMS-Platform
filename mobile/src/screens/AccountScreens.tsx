...
export function DashboardScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Dashboard'>) {
  ...
  return <Screen><View style={styles.welcome}><Text style={styles.hello}>Xin chào,</Text><Text style={styles.name}>{user.fullName}</Text><Text style={styles.role}>{roleName[user.role]}</Text></View>
    ...
    {user.role === 'STUDENT' && <Menu title="Đơn hàng của tôi" note="Thanh toán và xem lịch sử giao dịch" onPress={() => navigation.navigate('Orders')} />}
    {user.role === 'STUDENT' && <Menu title="Chứng chỉ của tôi" note="Xem các chứng chỉ đã được cấp" onPress={() => navigation.navigate('Certificates')} />}
    <Menu title="Xác minh chứng chỉ" note="Kiểm tra chứng chỉ công khai" onPress={() => navigation.navigate('VerifyCertificate')} />
