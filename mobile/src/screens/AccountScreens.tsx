...
export function DashboardScreen({ navigation }: { navigation: any }) {
  ...
  return <Screen topInset><AppBar title="Cá nhân" subtitle={roleName[user.role]} /><View style={accountPhaseStyles.profileHero}><View style={accountPhaseStyles.profileAvatar}>{user.avatarUrl ? <Image source={{ uri: user.avatarUrl }} style={accountPhaseStyles.profileAvatar} /> : <Text style={accountPhaseStyles.profileLetter}>{user.fullName.charAt(0).toUpperCase()}</Text>}</View><View style={{ flex: 1 }}><Text style={accountPhaseStyles.welcome}>Xin chào,</Text><Text style={accountPhaseStyles.profileName}>{user.fullName}</Text><Text style={accountPhaseStyles.profileEmail}>{user.email}</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Chỉnh sửa hồ sơ" onPress={() => navigation.navigate('Profile')} style={accountPhaseStyles.editProfile}><Ionicons name="create-outline" size={19} color="#fff" /></Pressable></View>
    <View style={accountPhaseStyles.workspaceHeading}><View><Text style={accountPhaseStyles.workspaceTitle}>{workspaceTitle[user.role]}</Text><Text style={accountPhaseStyles.workspaceNote}>{workspaceNote[user.role]}</Text></View></View>
...
function AccountShortcut({ icon, title, note, tone, onPress }: AccountShortcutItem & { onPress(): void }) { return <Pressable accessibilityRole="button" onPress={onPress} style={accountPhaseStyles.shortcut}><View style={[accountPhaseStyles.shortcutIcon, { backgroundColor: `${tone}18` }]}><Ionicons name={icon} size={24} color={tone} /></View><Text style={accountPhaseStyles.shortcutTitle}>{title}</Text><Text style={accountPhaseStyles.shortcutNote}>{note}</Text><Ionicons name="arrow-forward-circle-outline" size={19} color={tone} style={accountPhaseStyles.shortcutArrow} /></Pressable>; }
function AccountSection({ title, children }: { title: string; children: ReactNode }) { return <View style={accountPhaseStyles.section}><Text style={accountPhaseStyles.sectionTitle}>{title.toUpperCase()}</Text><View style={accountPhaseStyles.sectionBody}>{children}</View></View>; }
function AccountRow({ icon, title, note, onPress }: { icon: keyof typeof Ionicons.glyphMap; title: string; note: string; onPress(): void }) { return <Pressable accessibilityRole="button" onPress={onPress} style={accountPhaseStyles.row}><View style={accountPhaseStyles.rowIcon}><Ionicons name={icon} size={20} color={colors.primary} /></View><View style={{ flex: 1 }}><Text style={accountPhaseStyles.rowTitle}>{title}</Text><Text style={accountPhaseStyles.rowNote}>{note}</Text></View><Ionicons name="chevron-forward" size={20} color="#a0a5b2" /></Pressable>; }
