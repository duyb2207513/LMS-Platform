...
export function LearningScreen({ route, navigation }: NativeStackScreenProps<RootStackParamList, 'Learning'>) {
  ...
  return <Screen>
    ...
    <View style={styles.learningActions}><View style={{ flex: 1 }}><Button title="Chương & bài" variant="outline" onPress={() => setSidebar(true)} /></View><View style={{ flex: 1 }}><Button title="Bài tập" variant="outline" onPress={() => navigation.navigate('Assignments', { courseId, courseTitle })} /></View></View>
    <Button title="Thông báo khóa học" variant="ghost" onPress={() => navigation.navigate('Announcements', { courseId, courseTitle })} />
    {!selected ? <StateView empty="Khóa học chưa có bài học đã xuất bản" /> : <View style={styles.viewerCard} {...lessonSwipe.panHandlers}>
      ...
      <View style={styles.lessonNav}>{lessons.findIndex(item => item.id === selected.id) > 0 && <Text style={styles.navLink} onPress={() => selectLesson(lessons[lessons.findIndex(item => item.id === selected.id) - 1])}>← Bài trước</Text>}<Text style={styles.swipeNote}>Vuốt để chuyển bài</Text>{lessons.findIndex(item => item.id === selected.id) < lessons.length - 1 && <Text style={styles.navLink} onPress={() => selectLesson(lessons[lessons.findIndex(item => item.id === selected.id) + 1])}>Bài sau →</Text>}</View>
    </View>}
